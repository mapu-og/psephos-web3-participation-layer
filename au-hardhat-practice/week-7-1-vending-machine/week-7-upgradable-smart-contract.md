...
Make sure it's from an app on the Sepolia test network! 👀

Your Sepolia private key, exported from a wallet like MetaMask.
If you need testnet funds, use the sepolia faucet.
...
Run Deploy Script
In your project root folder, run npx hardhat run scripts/deployProxy.js --network sepolia
...
Go to Sepolia Etherscan and copy-paste your V1 Implementation address into the search bar
...
In your project root, run npx hardhat run scripts/upgradeProxy.js --network sepolia
...
Verify the VendingMachineV2 contract by running npx hardhat verify --network sepolia VendingMachineV2_CONTRACT_ADDRESS
